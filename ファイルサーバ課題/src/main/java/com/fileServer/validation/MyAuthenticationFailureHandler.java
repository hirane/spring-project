 ...
 * @since 3.0
 */
//SimpleUrlAuthenticationFailureHandlerクラスベースのユーザ認証エラーハンドリングクラス
...
//		setUserIdParam(userIParam);
	...
	public void onAuthenticationFailure(HttpServletRequest req, HttpServletResponse res,
		...
		//エラー内容によって異なる出力用メッセージを代入
		String errReason = null;
		if (e instanceof BadCredentialsException) {
			//ユーザIDがDBに存在していた場合
			if((boolean)session.getAttribute("existsId")) {
				errReason = "パスワードが間違っています";
			}
			//ユーザIDが存在しなかった場合
			else {
				errReason = "指定されたユーザは存在しません";
			}
		} else if(e instanceof AccountExpiredException) {
			errReason = "アカウントの有効期限が切れています";
		} else if(e instanceof CredentialsExpiredException) {
			errReason = "資格の有効期限が切れています";
		} else if(e instanceof DisabledException) {
			errReason = "アカウントが利用できません";
		} else if(e instanceof LockedException) {
			errReason = "アカウントがロックされています";
		} else {
			errReason = "その他の問題によりログインに失敗しました";
		}
		...
		if (this.forwardToDestination) {
			...
			this.redirectStrategy.sendRedirect(req, res, this.defaultFailureUrl);
		}
	}
	 ...
	 */
	protected final void saveException(HttpServletRequest req, AuthenticationException e) {
		if (this.forwardToDestination) {
			req.setAttribute(WebAttributes.AUTHENTICATION_EXCEPTION, e);
			return;
		}
		HttpSession session = req.getSession(false);
		if (session != null || this.allowSessionCreation) {
			req.getSession().setAttribute(WebAttributes.AUTHENTICATION_EXCEPTION, e);
		}
	}
...
public class MyAuthenticationFailureHandler implements AuthenticationFailureHandler {
	...
	public void onAuthenticationFailure(HttpServletRequest req, HttpServletResponse res,
		...
		String errReason = null;
		if (e instanceof BadCredentialsException) {
		    errReason = "ユーザが存在しないか、パスワードが間違っています";
		} else if(e instanceof AccountExpiredException) {
		    errReason = "アカウントの有効期限が切れています";
		} else if(e instanceof CredentialsExpiredException) {
		    errReason = "資格の有効期限が切れています";
